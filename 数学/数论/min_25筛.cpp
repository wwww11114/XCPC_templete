...
i64 min25_sieve(i64 n) {
    ...
    auto solve_G = [&](auto F, auto SF) {
        ...
        for (auto p : pri) {
            ...
            for (int it = (int)v.size() - 1; v[it] >= 1ll * p * p; it--) {
                i64 i = v[it];
                G[id(i)] = (G[id(i)] - F(p) * (G[id(i / p)] - pre + mod) % mod + mod) % mod;
            }
            pre = (pre + F(p)) % mod;
        }
        return G;
    ...
    auto solve_S = [&](vector<i64> &G, auto F) -> i64 {
        ...
        auto dfs = [&](auto &&dfs, i64 n, int x, i64 pre) -> i64 {
            ...
            i64 ans = (G[id(n)] - pre + mod) % mod;
            for (int i = x + 1; i <= pri.size() && 1ll * pri[i - 1] * pri[i - 1] <= n; i++) {
                pre = (pre + F(pri[i - 1])) % mod;
                for (i64 p = pri[i - 1]; p <= n; p *= pri[i - 1]) {
                    ans = (ans + F(p) * (dfs(dfs, n / p, i, pre) + (p != pri[i - 1]))) % mod;
                }
            }
            return ans;
        };
        return (dfs(dfs, n, 0, 0) + 1) % mod;
    };
    ...
    auto G1 = solve_G(
        ...
        [](i64 x) {
        x = x % mod;
        return (x * (x + 1) % mod * inv2 - 1 + mod) % mod;
    }
    );
    auto G = solve_G(
        [](i64 x) {
        ...
        [](i64 x) {
        x = x % mod;
        return (x * (x + 1) % mod * (2 * x + 1) % mod * inv6 - 1 + mod) % mod;
    }
    );
    for (int i = 0; i < G.size(); i++) {
        G[i] = (G[i] - G1[i] + mod) % mod;
    }
